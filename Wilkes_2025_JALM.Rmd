...
df_stability <- read.csv("~/Data/WilkesEH_JALM_stability_data.csv")
df_temp <- read.csv("~/Data/WilkesEH_JALM_temperature_data.csv")
df_time <- read.csv("~/Data/WilkesEH_JALM_transit_time_data.csv")
```
...
model_stability_slopes <- brms::brm(
  formula = concentration ~ 0 + temperature + (0 + temperature | method) + (1 | method/lab) + (1 | sample)
  ,data = df_stability
  ,family = student
  ,prior = c(
...
model_temp_slopes <- brm(
  concentration ~ 0 + temperature + (0 + temperature | method) + (1 | method/lab)
  ,data = df_temp
  ,family = student
  ,cores = 4
  ,iter = 4000
